...
export const foundRestaurants = restaurants => ({
...
export const setFoundRestaurantIndex = number => ({
  type: SET_FOUND_RESTAURANT_INDEX,
  number
})
export const resetRestaurauntIndex = () => ({
...
export default function reducer(restaurants = {
  ...
  switch (action.type) {
    ...
    case GET_FAVORITES:
      return Object.assign({}, restaurants, {
        favorites: [...restaurants.favorites, action.favorites]
      })
...
export const favoriteDish = (terms) => (dispatch) => {
  axios.post(`/api/users/${terms.currentUser.id}/favorites`, {
      terms
    })
...
export const searchRestaurants = (searchTerms, history) => dispatch => {
  ...
  axios.post('/api/restaurants/yelp', searchTerms)
    ...
    .catch(error => {
      console.log(error)
      dispatch(foundRestaurants(['none']))
    })
}